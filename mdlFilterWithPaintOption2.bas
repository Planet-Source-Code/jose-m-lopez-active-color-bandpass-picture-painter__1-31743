Attribute VB_Name = "mdlFilterWithPaintOption2"
Public Function fncRGB() As Long
fncRGB = RGB(rAdj * rOnOff, gAdj * gOnOff, bAdj * bOnOff)
End Function

Public Sub rtnFilterWithPaintOption2()
...
'We paint according to the current R, G, or B
'and mark this pixel as painted.
...
'They are painted according to the current R, G, or B
...
'Then paint it to the screen according to current RGB
.picMain.PSet (intPxlX, intPxlY), fncRGB()
       ...
       'Get its XY coordinates
        ...
        color = lngPixelColors(X + 1, Y)
            ...
            intColorToFilter = rAdj + bAdj + gAdj
                ...
                'Paint it to the screen according to current RGB
                .picMain.PSet (X + 1, Y), fncRGB()
