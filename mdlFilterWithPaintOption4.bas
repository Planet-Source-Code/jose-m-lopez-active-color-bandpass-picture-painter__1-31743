Attribute VB_Name = "mdlFilterWithPaintOption4"
Public Function fncBWplusRGB() As Long
intBW = intColorToFilter / 3
rDiff = rAdj - intBW: gDiff = gAdj - intBW: bDiff = bAdj - intBW
fncBWplusRGB = RGB(intBW + (rDiff * rOnOff), intBW + (gDiff * gOnOff), intBW + (bDiff * bOnOff))
End Function

Public Sub rtnFilterWithPaintOption4()
...
'We greyscale paint and mark this pixel as painted.
'Before we paint it though we superimpose
'either the B, GB, or RGB values. Depending on the current values
'of our OnOff variables determined right at our cmdGo button click.
...
'greyscale painted.
'As before, before we paint them though we superimpose
'either the B, GB, or RGB values. Depending on the current values
'of our OnOff variables determined right at our cmdGo button click.
...
'Greyscale Paint it and superimpose either B,GB, or RGB color
.picMain.PSet (intPxlX, intPxlY), fncBWplusRGB()
       ...
       'Get its XY coordinates
        ...
        color = lngPixelColors(X + 1, Y)
            ...
            intColorToFilter = rAdj + bAdj + gAdj
                ...
                'Greyscale Paint it and superimpose either B,GB, or RGB color
                .picMain.PSet (X + 1, Y), fncBWplusRGB()
        ...
        color = lngPixelColors(X, Y + 1)
            ...
            intColorToFilter = rAdj + bAdj + gAdj
                ...
                'Greyscale Paint it and superimpose either B,GB, or RGB color
                .picMain.PSet (X, Y + 1), fncBWplusRGB()
                'Update/increment unchecked for adjacents pixel count
                UnChkdForAdjPxlCount = UnChkdForAdjPxlCount + 1
                'Update unchecked for adjacents pixel array
        ...
        color = lngPixelColors(X + 1, Y + 1)
            ...
            intColorToFilter = rAdj + bAdj + gAdj
                ...
                'Greyscale Paint it and superimpose either B,GB, or RGB color
                .picMain.PSet (X + 1, Y + 1), fncBWplusRGB()
                'Update/increment unchecked for adjacents pixel count
                UnChkdForAdjPxlCount = UnChkdForAdjPxlCount + 1
                'Update unchecked for adjacents pixel array
