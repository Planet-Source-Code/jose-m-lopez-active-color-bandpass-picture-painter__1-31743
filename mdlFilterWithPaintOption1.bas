Attribute VB_Name = "mdlFilterWithPaintOption1"
Public lngPixelColors() As Long  'Holds the raster's individual pixel colors
Public lngPixelColorsBkp() As Long  'Holds the raster's individual pixel colors
Public arrAdjPxlsToChk() As Integer 'Holds XYs of adjacent pixels not yet checked
Public arrAdjPxlsBeingChkd() As Integer 'Holds XYs of adjacent pixels being checked
Public intRandomXYs() As Integer 'Holds the raster's individual X, Y coordinates in random order
Public lngTotalPixelCount As Long 'Holds the raster's total pixel count
Public rAdj As Integer, gAdj As Integer, bAdj As Integer, intColorToFilter As Integer
Public rOnOff As Integer, gOnOff As Integer, bOnOff As Integer
Public color As Long
Public intColorFilter As Integer
Public strFileName As String 'Picture's location
Public strFileNameBkp As String 'Picture's location
Public picFilename As Picture 'Picture in memory
Public lngDelay As Long 'To adjust our painting speed
Public Const cAlreadyPainted = 20000000 'Impossible color value used to mark processed pixels
Public Const cX = 0 'Where in array X coordinates are located (Simplify readability)
Public Const cY = 1 'Where in array Y coordinates are located (Simplify readability)
'****************************************************************************

Public Sub rtnGetRGBColors()
rAdj = color Mod 256
bAdj = Int(color / 65536)
gAdj = (color - (bAdj * 65536) - rAdj) / 256
End Sub

Public Sub rtnFilterWithPaintOption1()
...
'if they pass the filter test.If yes they are marked and painted.
...
.picMain.PSet (intPxlX, intPxlY), colortrace
...
'cmdGo and filter text box enabled only when necessary
End With
